 ...
 *
 * @package		CodeIgniter
 * @author		ExpressionEngine Dev Team$this->orig_height
...
class CI_Image_lib {
	 ...
	 * @return	bool
	 */
        function compress($maximum_size) 
        {
            if($this->orig_width > 1000 || $this->orig_height > 1000)
            {
		...
		else
		{
			...
			$copy	= 'imagecopyresized';
                ...
                list($width, $height) = $this->_reproportion(1000, 1000);
                
		$dst_img = $create($width, $height);
		$copy($dst_img, $src_img, 0, 0, 0, 0, $width, $height, $this->orig_width, $this->orig_height);
                
                $this->image_save_gd($dst_img, $this->full_src_path);
            ...
            $xp = $this->explode_name($this->full_src_path);
            $filename = $xp['name'];
            $file_ext = strtolower($xp['ext']);
            $quality = 75;

            if (!in_array($file_ext, array('.jpg', '.jpeg'))) 
            {
                $src_file = $this->full_src_path;
                $dst_file = $filename . '.jpg';
                $rm = $this->full_src_path;
            } 
            else 
            {
                $src_file = $dst_file = $this->full_src_path;
            }
            
            $file_size = round($this->get_real_filesize($src_file) / 1024, 2);

            do {
                $src_img = $this->image_create_gd($src_file, $file_type);

                @imagejpeg($src_img, $dst_file, $quality);
                @imagedestroy($src_img);

                $file_size = round($this->get_real_filesize($dst_file) / 1024, 2);
                $src_file = $dst_file;
                $file_type = 2;
                $quality -= 10;
            ...
            if (isset($rm)) 
            {
                ...
                $this->source_image = $src_file;
                // Reinitialize for other oparetion
                $this->initialize();
            }
            return TRUE;
        }
	 ...
	 * @return	bool
	 */
	function image_save_gd($resource, $dst_path = NULL)
	{
                if(! isset($dst_path))
                {
                        $dst_path = $this->full_dst_path;
                }
		switch ($this->image_type)
		{
			case 1 :
			...
			case 2	:
						...
						if ( ! @imagejpeg($resource, $dst_path, $this->quality))
						{
							$this->set_error('imglib_save_failed');
							return FALSE;
						}
	...
	function image_reproportion()
	{
        ...
        function _reproportion($width, $height)
	{
		$new_width	= ceil($this->orig_width*$height/$this->orig_height);
		$new_height	= ceil($width*$this->orig_height/$this->orig_width);

		$ratio = (($this->orig_height/$this->orig_width) - ($height/$width));
                $dim = ($ratio < 0) ? 'width' : 'height';
